...
class TestViewsetTestCaseInSession(TestApi):
    ...
    def _create_testcaseinsession(self, expected_status):
        response = self.client.post('/snapshot/api/testcaseinsession/',
                                    data={'name': 'bla', 'testCase': 1, 'session': 1, 'status': 'SUCCESS',
                                          'testSteps': [2, 3]})
    ...
    def test_testcaseinsession_create_with_model_permission(self):
        ...
        self._create_and_authenticate_user_with_permissions(Permission.objects.filter(
            Q(codename='add_testcaseinsession', content_type=self.content_type_testcaseinsession)))
    ...
    def test_testcaseinsession_other_verbs_forbidden(self):
        ...
        """
        self._create_and_authenticate_user_with_permissions(Permission.objects.filter(
            Q(codename='view_testcaseinsession', content_type=self.content_type_testcaseinsession)
    ...
    def test_testcaseinsession_create_with_application_restriction_and_change_permission(self):
        ...
        self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='change_testcaseinsession')))
        self._create_testcaseinsession(403)
    ...
    def test_creation_when_exist_with_test_steps(self):
        ...
        self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='can_view_application_myapp')))
        response = self.client.post('/snapshot/api/testcaseinsession/',
                                    data={'session': 8, 'testCase': 4, 'testSteps': [2, 3, 4]})
        self.assertEqual(response.status_code, 201)
        self.assertTrue('id' in response.data)
    ...
    def test_no_creation_when_exist_with_many_to_many_fields(self):
        ...
        self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='can_view_application_myapp')))
        response = self.client.post('/snapshot/api/testcaseinsession/',
                                    data={'session': 7, 'testCase': 4, 'testSteps': [2, 3, 4]})
        self.assertEqual(response.status_code, 201)
        self.assertTrue('id' in response.data)
