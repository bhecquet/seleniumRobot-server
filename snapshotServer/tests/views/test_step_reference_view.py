...
class TestStepReferenceView(TestApi):
    ...
    def setUp(self):
        ...
        self.content_type_application = ContentType.objects.get_for_model(variableServer.models.Application,
                                                                          for_concrete_model=False)
        self.content_type_environment = ContentType.objects.get_for_model(variableServer.models.TestEnvironment,
                                                                          for_concrete_model=False)
    ...
    def test_get_snapshot_security_authenticated_no_permission_on_application(self):
        ...
        self._create_and_authenticate_user_with_permissions(Permission.objects.filter(
            Q(codename='can_view_application_myapp2', content_type=self.content_type_application)))
    ...
    def test_get_snapshot_security_authenticated_with_permission_on_application(self):
        ...
        self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='can_view_application_myapp', content_type=self.content_type_application)))
    ...
    def test_get_snapshot_security_authenticated_with_permission_on_environment(self):
        ...
        self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='can_view_environment_DEV', content_type=self.content_type_environment)))
    ...
    def test_get_snapshot_security_authenticated_with_permission_on_other_environment(self):
        ...
        self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='can_view_environment_PROD', content_type=self.content_type_environment)))
    ...
    def test_post_snapshot_no_ref(self):
        ...
        with open('snapshotServer/tests/data/replyDetection.json.png', 'rb') as fp:
            ...
            uploaded_reference = StepReference.objects.filter(testCase=self.tcs1.testCase, testStep__id=1,
                                                              version=Version.objects.get(pk=1),
                                                              environment=TestEnvironment.objects.get(id=1)).last()
    ...
    def test_post_snapshot_clean_old_reference(self):
        ...
        with open('snapshotServer/tests/data/replyDetection.json.png', 'rb') as fp:
            ...
            uploaded_reference_1 = StepReference.objects.filter(testCase=self.tcs1.testCase, testStep__id=1,
                                                                version=Version.objects.get(pk=1),
                                                                environment=TestEnvironment.objects.get(id=1)).last()
        ...
        with open('snapshotServer/tests/data/replyDetection2.json.png', 'rb') as fp:
            response = self.client.post(reverse('uploadStepRef'),
                                        data={'stepResult': self.step_result_same_env.id, 'image': fp})
            self.assertEqual(response.status_code, 201, 'status code should be 201: ' + str(response.content))
            time.sleep(0.5)  # wait field computing

            uploaded_reference_2 = StepReference.objects.filter(testCase=self.tcs_same_env.testCase, testStep__id=1,
                                                                version=Version.objects.get(pk=1),
                                                                environment=TestEnvironment.objects.get(id=1)).last()
    ...
    def test_post_snapshot_existing_ref_other_env(self):
        ...
        with open('snapshotServer/tests/data/engie.png', 'rb') as fp:
            response = self.client.post(reverse('uploadStepRef'),
                                        data={'stepResult': self.step_result_other_env.id, 'image': fp})
            self.assertEqual(response.status_code, 201, 'status code should be 201: ' + str(response.content))
            time.sleep(0.5)  # wait field computing

            uploaded_reference_2 = StepReference.objects.filter(testCase=self.tcs_other_env.testCase,
                                                                version=Version.objects.get(pk=1),
    ...
    def test_post_snapshot_existing_ref_other_version(self):
        ...
        with open('snapshotServer/tests/data/engie.png', 'rb') as fp:
            response = self.client.post(reverse('uploadStepRef'),
                                        data={'stepResult': self.step_result_other_version.id, 'image': fp})
            self.assertEqual(response.status_code, 201, 'status code should be 201: ' + str(response.content))
            time.sleep(0.5)  # wait field computing

            uploaded_reference_2 = StepReference.objects.filter(testCase=self.tcs_other_version.testCase,
    ...
    def test_post_snapshot_missing_step(self):
        ...
        with open('snapshotServer/tests/data/engie.png', 'rb') as fp:
            ...
            self.assertEqual(response.status_code, 403,
                             'status code should be 403')  # as step result is not provided, it's not possible to check permissions and we get permission denied
