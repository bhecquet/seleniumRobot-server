...
class TestVariableAdmin(TestWebAndAdmin):
    ...
    def test_variable_queryset_with_application_and_environment_restriction(self):
        ...
        user, client = self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='can_view_application_app1') | Q(codename='can_view_environment_DEV')))
        ...
        self.assertEqual(len(query_set), Variable.objects.filter(
            Q(application=expected_application) | Q(environment=expected_environment)).distinct().count())
    ...
    def test_variable_queryset_with_application_restriction_and_view_variable_permission(self):
        ...
        user, client = self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='view_variable')))
    ...
    def test_variable_save_standard_with_csv_file(self):
        """
    ...
    def test_variable_save_standard_with_xlsx_file(self):
        ...
        if os.path.exists(result_file_path):
            os.remove(result_file_path)
        with open(file_path, 'rb') as f:
            in_memory_uploaded_file = InMemoryUploadedFile(f, 'uploadFile', 'filetoadd.xlsx',
    ...
    def test_variable_clean_no_concurrent_file_value(self):
        ...
        form.cleaned_data = {"name": "both", "value": "filetxt",
                             "uploadFile": "itssupposedtobeafilebutaslongasitsnotnullitsokforthetest"}
        self.assertRaisesRegex(ValidationError, ".*A variable can't be both a value and a file. Choose only one..*",
                               form.clean)
    ...
    def test_variable_clean_file_wrong_type(self):
        ...
        with open(file_path, 'rb') as f:
            in_memory_uploaded_file = InMemoryUploadedFile(f, 'uploadFile', 'engie.png', 'image/png',
                                                           os.path.getsize(file_path), None)
            form = VariableForm(data={'name': 'foo'}, files={'uploadFile': in_memory_uploaded_file})
            self.assertFalse(form.is_valid())
            self.assertRaisesRegex(ValidationError,
                                   ".*is an unsupported file type. Please, select csv,xls,xlsx,json,txt,pdf file..*",
                                   form.clean)
    ...
    def test_variable_clean_file_wrong_type_txt(self):
        ...
        with open(file_path, 'rb') as f:
            in_memory_uploaded_file = InMemoryUploadedFile(f, 'uploadFile', 'dummy.txt', 'text/plain',
                                                           os.path.getsize(file_path), None)
            form = VariableForm(data={'name': 'foo'}, files={'uploadFile': in_memory_uploaded_file})
            self.assertFalse(form.is_valid())
            self.assertRaisesRegex(ValidationError,
    ...
    def test_variable_clean_file_too_large(self):
        ...
        with open(file_path, 'rb') as f:
            in_memory_uploaded_file = InMemoryUploadedFile(f, 'uploadFile', 'toobig.csv', 'text/csv',
                                                           os.path.getsize(file_path), None)
            form = VariableForm(data={'name': 'foo'}, files={'uploadFile': in_memory_uploaded_file})
            self.assertFalse(form.is_valid())
    ...
    def test_variable_as_file_change_file(self):
        ...
        with open(new_file_path, 'rb') as f:
            in_memory_uploaded_file = InMemoryUploadedFile(f, 'uploadFile', 'replacement.csv', 'text/csv',
                                                           os.path.getsize(new_file_path), None)
    ...
    def test_variable_save_protected_variable_with_authorized_user(self):
        ...
        user, client = self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='see_protected_var')))
    ...
    def test_variable_copy_to_no_variables(self):
        ...
        user, client = self._create_and_authenticate_user_with_permissions(Permission.objects.filter(
            Q(codename='view_variable') | Q(codename='change_variable') | Q(codename='add_variable')))
    ...
    def _test_variable_unreserve(self, permissions, app_of_application):
        ...
        # reserve variable
        reservable_var = Variable(name='var1', value='value1', application=app_of_application,
                                  version=Version.objects.get(pk=1),
                                  releaseDate=timezone.now() + datetime.timedelta(seconds=60))
        reservable_var.save()
    ...
    def test_variable_unreserve(self):
        ...
        """
        reservable_var = self._test_variable_unreserve(Permission.objects.filter(
            Q(codename='view_variable') | Q(codename='change_variable') | Q(codename='add_variable')),
                                                       Application.objects.get(pk=1))
    ...
    def test_variable_unreserve_no_change_permission(self):
        ...
        Check it's possible to unreserve a variable when global variable permission are given to user
        """
        reservable_var = self._test_variable_unreserve(
            Permission.objects.filter(Q(codename='view_variable') | Q(codename='add_variable')),
            Application.objects.get(pk=1))
    ...
    def test_variable_unreserve_with_application_restrictions(self):
        ...
        reservable_var = self._test_variable_unreserve(Permission.objects.filter(Q(codename='can_view_application_app1')),
                                                       Application.objects.get(pk=1))
    ...
    def test_variable_unreserve_with_application_restrictions_and_global_change_variable(self):
        ...
        reservable_var = self._test_variable_unreserve(Permission.objects.filter(Q(codename='change_variable')),
                                                       Application.objects.get(pk=1))
    ...
    def test_variable_unreserve_with_application_restrictions_and_without_global_change_variable(self):
        ...
        user, client = self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='add_variable')))

        # reserve variable
        reservable_var = Variable(name='var1', value='value1', application=Application.objects.get(pk=1),
                                  version=Version.objects.get(pk=1),
                                  releaseDate=timezone.now() + datetime.timedelta(seconds=60))
        reservable_var.save()
    ...
    def test_variable_change_values_at_once_with_application_restrictions(self):
        ...
        content = self._test_variable_change_values_at_once(
            Permission.objects.filter(Q(codename='can_view_application_app1')),
            [3, 4, 9, 10])  # 3 & 4: app1; 9: no app; 10: app3,
    ...
    def test_variable_form_with_existing_variable_no_application_defined(self):
        ...
        self.assertEqual(form.fields['test'].help_text,
                         "Select an application, click 'save and continue editing' to display the list of related tests")
        ...
        self.assertEqual(form.fields['version'].help_text,
                         "Select an application, click 'save and continue editing' to display the list of related versions")
    ...
    def test_user_can_see_variables_without_global_rights_and_application_permissions(self):
        ...
        self.assertFalse(
            variable_admin.has_view_permission(request=MockRequest(user=user), obj=Variable.objects.get(pk=301)))
        ...
        self.assertTrue(
            variable_admin.has_change_permission(request=MockRequest(user=user), obj=Variable.objects.get(pk=3)))
        self.assertFalse(
            variable_admin.has_change_permission(request=MockRequest(user=user), obj=Variable.objects.get(pk=301)))
        ...
        self.assertTrue(
            variable_admin.has_delete_permission(request=MockRequest(user=user), obj=Variable.objects.get(pk=3)))
        self.assertFalse(
            variable_admin.has_delete_permission(request=MockRequest(user=user), obj=Variable.objects.get(pk=301)))
    ...
    def test_user_can_see_variables_with_application_restrictions_and_view_permission(self):
        ...
        self.assertFalse(
            testcase_admin.has_change_permission(request=MockRequest(user=user), obj=Variable.objects.get(pk=1)))
        self.assertFalse(
            testcase_admin.has_change_permission(request=MockRequest(user=user), obj=Variable.objects.get(pk=3)))
        ...
        self.assertFalse(
            testcase_admin.has_delete_permission(request=MockRequest(user=user), obj=Variable.objects.get(pk=1)))
        self.assertFalse(
            testcase_admin.has_delete_permission(request=MockRequest(user=user), obj=Variable.objects.get(pk=3)))
    ...
    def test_user_can_change_variables_with_application_restrictions_and_change_permission(self):
        ...
        testcase_admin = VariableAdmin(model=Variable, admin_site=AdminSite())
        user, client = self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='change_variable')))
        self.assertTrue(testcase_admin.has_view_permission(request=MockRequest(user=user)))
        self.assertTrue(testcase_admin.has_view_permission(request=MockRequest(user=user), obj=Variable.objects.get(pk=1)))
        self.assertTrue(testcase_admin.has_view_permission(request=MockRequest(user=user), obj=Variable.objects.get(pk=3)))
        self.assertFalse(testcase_admin.has_add_permission(request=MockRequest(user=user)))
