...
class TestVersionAdmin(TestWebAndAdmin):
    ...
    def test_version_filter_lookup_without_application(self):
        ...
        self.assertEqual(filtered_versions,
                         [(1, 'app1-2.4'), (2, 'app1-2.5'), (3, 'app2-1.0'), (4, 'app3-1.0'), (5, 'app4-1.0'),
                          (6, 'linkedApp4-1.0'), (7, 'app5NoVar-1.0'), ('_None_', 'None')])
    ...
    def test_version_has_delete_permission_allowed_and_authenticated_without_linked_variables(self):
        ...
        user, client = self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='delete_version', content_type=self.content_type_version)))
        ...
        self.assertTrue(version_admin.has_delete_permission(request=MockRequest(user=user),
                                                            obj=Version.objects.filter(pk=version_no_variable.id)[
                                                                0]))  #
    ...
    def test_version_has_delete_permission_allowed_and_authenticated_with_linked_variables(self):
        ...
        self.assertFalse(version_admin.has_delete_permission(request=MockRequest(user=user),
                                                             obj=Version.objects.filter(pk=version_with_variables.id)[
                                                                 0]))  # 'app1-2.5' has only variable linked to it
    ...
    def test_version_has_delete_permission_allowed_and_authenticated_with_application_restriction_and_application_permission(
            self):
        """
        delete version 
        user:
    ...
    def test_version_has_delete_permission_allowed_and_authenticated_with_application_restriction_and_delete_permission(
            self):
        """
        delete version
        user:
    ...
    def test_version_has_delete_permission_allowed_and_authenticated_with_application_restriction_and_change_permission(
            self):
        """
        delete version
        user:
        ...
        version_admin = VersionAdmin(model=Version, admin_site=AdminSite())
        user, client = self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='change_version', content_type=self.content_type_version)))
    ...
    def test_version_queryset_with_application_restriction_and_global_view_version(self):
        ...
        user, client = self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='view_version', content_type=self.content_type_version)))
    ...
    def test_user_can_see_versions_without_global_rights_and_application_permissions(self):
        ...
        self.assertTrue(testcase_admin.has_view_permission(request=MockRequest(user=user),
                                                           obj=Version.objects.get(pk=1)))  # version from app1
        self.assertFalse(testcase_admin.has_view_permission(request=MockRequest(user=user),
                                                            obj=Version.objects.get(pk=3)))  # version from app2
        ...
        self.assertFalse(
            testcase_admin.has_change_permission(request=MockRequest(user=user), obj=Version.objects.get(pk=3)))
        ...
        self.assertFalse(testcase_admin.has_delete_permission(request=MockRequest(user=user), obj=Version.objects.get(
            pk=1)))  # cannot delete as version has linked variables
        self.assertFalse(
            testcase_admin.has_delete_permission(request=MockRequest(user=user), obj=Version.objects.get(pk=3)))
    ...
    def test_user_can_see_versions_with_application_restrictions_and_view_permission(self):
        ...
        testcase_admin = VersionAdmin(model=Version, admin_site=AdminSite())
        user, client = self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='view_version')))
        self.assertTrue(testcase_admin.has_view_permission(request=MockRequest(user=user)))
        ...
        self.assertFalse(
            testcase_admin.has_change_permission(request=MockRequest(user=user), obj=Version.objects.get(pk=1)))
        self.assertFalse(
            testcase_admin.has_change_permission(request=MockRequest(user=user), obj=Version.objects.get(pk=3)))
        ...
        self.assertFalse(
            testcase_admin.has_delete_permission(request=MockRequest(user=user), obj=Version.objects.get(pk=1)))
        self.assertFalse(
            testcase_admin.has_delete_permission(request=MockRequest(user=user), obj=Version.objects.get(pk=3)))
    ...
    def test_user_can_add_versions_with_application_restrictions_and_add_permission(self):
        ...
        testcase_admin = VersionAdmin(model=Version, admin_site=AdminSite())
        user, client = self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='add_version')))
        self.assertFalse(testcase_admin.has_view_permission(request=MockRequest(user=user)))
    ...
    def test_user_can_change_versions_with_application_restrictions_and_change_permission(self):
        ...
        testcase_admin = VersionAdmin(model=Version, admin_site=AdminSite())
        user, client = self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='change_version')))
        self.assertTrue(testcase_admin.has_view_permission(request=MockRequest(user=user)))
        self.assertTrue(testcase_admin.has_view_permission(request=MockRequest(user=user), obj=Version.objects.get(pk=1)))
        self.assertTrue(testcase_admin.has_view_permission(request=MockRequest(user=user), obj=Version.objects.get(pk=3)))
        self.assertFalse(testcase_admin.has_add_permission(request=MockRequest(user=user)))
    ...
    def test_user_can_delete_versions_with_application_restrictions_and_delete_permission(self):
        ...
        testcase_admin = VersionAdmin(model=Version, admin_site=AdminSite())
        user, client = self._create_and_authenticate_user_with_permissions(
            Permission.objects.filter(Q(codename='delete_version')))
        self.assertFalse(testcase_admin.has_view_permission(request=MockRequest(user=user)))
        self.assertFalse(testcase_admin.has_view_permission(request=MockRequest(user=user), obj=Version.objects.get(pk=1)))
        self.assertFalse(testcase_admin.has_view_permission(request=MockRequest(user=user), obj=Version.objects.get(pk=3)))
        self.assertFalse(testcase_admin.has_add_permission(request=MockRequest(user=user)))
        ...
        self.assertTrue(testcase_admin.has_delete_permission(request=MockRequest(user=user)))
        self.assertFalse(testcase_admin.has_delete_permission(request=MockRequest(user=user), obj=Version.objects.get(
            pk=1)))  # this version has linked variables and so cannot be deleted
